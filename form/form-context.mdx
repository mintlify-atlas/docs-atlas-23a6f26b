---
title: "Form Context and useFormContext in ShaddyForm"
sidebarTitle: "Form Context"
description: "FormContext provides react-hook-form's form instance to all child field components in ShaddyForm, enabling type-safe field registration without prop drilling."
---

When `ShaddyForm` renders, it wraps its children with `react-hook-form`'s `Form` provider, making the form instance available throughout the component tree. Field components tap into this context to register themselves, display validation errors, and read current values — all without receiving any form props directly from their parent.

## How FormContext Works

`ShaddyForm` calls `useForm()` internally and wraps its children with `react-hook-form`'s `<Form>` context provider (the `shadcn/ui` wrapper around `FormProvider`). Any component inside `ShaddyForm` can call `useFormContext()` to access the full `UseFormReturn` object — including `control`, `register`, `getValues`, `setValue`, `formState`, and more.

Shaddy also exports a typed `useShaddyFormContext` hook that reads a narrower context shape (`{ control }`) with a helpful error if called outside of a form:
...
<Note>
  In practice, most built-in field components use `react-hook-form`'s own `useFormContext()` directly, which provides the complete `UseFormReturn` API rather than just `control`. Use `useShaddyFormContext` when you only need the `control` object and want the built-in safety check.
</Note>
